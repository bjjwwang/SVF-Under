...
class AbstractExecution:
    ...
    def handleCallSite(self, node: pysvf.CallICFGNode):
        ...
        if fun_name == "svf_assert" or fun_name == "svf_assert_eq":
            ...
            # arg0 is the lower bound, arg1 is the upper bound
            lhs_id = node.getRetICFGNode().getActualRet().getId()
            arg0 = node.getArgument(0).getId()
            arg1 = node.getArgument(1).getId()
            abstract_state = self.post_abs_trace[node]
            if abstract_state[arg0].getInterval().isTop() or abstract_state[arg1].getInterval().isTop():
                self.post_abs_trace[node][lhs_id] = AbstractValue(IntervalValue.top())
            else:
                self.post_abs_trace[node][lhs_id] = AbstractValue(IntervalValue(int(abstract_state[arg0].getInterval().lb()), int(abstract_state[arg1].getInterval().ub())))
    ...
    def handleStubFunction(self, callNode: pysvf.CallICFGNode):
        ...
        if callNode.getCalledFunction().getName() == "svf_assert":
            ...
            if abstract_state[arg0].getInterval().isTop():
                ...
                # Under Approx: loose assertion.
                # TODO: maybe we need another API like may_assert to check if the assertion is loose.
                if abstract_state[arg0].getInterval().equals(IntervalValue(0, 1)):
                    print(f"The assertion ({callNode}) is successfully verified!!")
                    ## TODO: trigger under approximation 
                    self.triggerUnderApproximation(self.icfg.getGlobalICFGNode(),callNode)
